...
fn parse_raydium_amm_v4_swap(
    ...
    if instruction.accounts.len() < 12 {
        warn!("⚠️  AMM V4 Parse REJECTED: instruction.accounts.len()={} < 12 required", instruction.accounts.len());
    ...
    info!("✅ EXTRACTED POOL: {} | DEX: RaydiumAmmV4 | From ix accounts[0]", pool_address);
    ...
    if instruction.data.len() < 17 {
        warn!("⚠️  AMM V4 Parse REJECTED: instruction.data.len()={} < 17 required for amounts", instruction.data.len());
    ...
    if instruction.data[0] != 9 {
        warn!("⚠️  AMM V4 Parse REJECTED: Unknown discriminator {} (expected 9)", instruction.data[0]);
...
fn parse_raydium_clmm_swap(
    ...
    if instruction.data.len() < 24 {
        warn!("⚠️  CLMM Parse REJECTED: instruction.data.len()={} < 24 required", instruction.data.len());
        return None;
    }
    ...
    if instruction.accounts.len() < 8 {
        warn!("⚠️  CLMM Parse REJECTED: instruction.accounts.len()={} < 8 required", instruction.accounts.len());
        return None;
    }
    ...
    info!("✅ EXTRACTED POOL: {} | DEX: Raydium_CLMM | From ix accounts[0]",
          pool_address);
...
fn parse_raydium_cpmm_swap(
    ...
    if instruction.data.len() < 24 {
        warn!("⚠️  CPMM Parse REJECTED: instruction.data.len()={} < 24 required", instruction.data.len());
        return None;
    }
    ...
    if instruction.accounts.len() < 8 {
        warn!("⚠️  CPMM Parse REJECTED: instruction.accounts.len()={} < 8 required", instruction.accounts.len());
        return None;
    }
    ...
    info!("✅ EXTRACTED POOL: {} | DEX: RaydiumCpmm | From ix accounts[0]", pool_address);
...
fn parse_orca_whirlpool_swap(
    ...
    if instruction.data.len() < 24 {
        warn!("⚠️  Orca Parse REJECTED: instruction.data.len()={} < 24 required", instruction.data.len());
        return None;
    }
    ...
    if instruction.accounts.len() < 11 {
        warn!("⚠️  Orca Parse REJECTED: instruction.accounts.len()={} < 11 required", instruction.accounts.len());
        return None;
    }

    // 🔍 COMPREHENSIVE DIAGNOSTIC: Log ALL instruction accounts and their resolved addresses
    info!("🔍🔍🔍 ORCA PARSE DIAGNOSTIC START 🔍🔍🔍");
    info!("  instruction.accounts.len() = {}", instruction.accounts.len());
    ...
    for (i, &account_idx) in instruction.accounts.iter().enumerate().take(11) {
        if let Some(account) = accounts.get(account_idx as usize) {
            ...
            warn!("  ix.accounts[{}] = {} -> OUT OF BOUNDS (accounts.len={})", i, account_idx, accounts.len());
        }
    }
    ...
    let pool_address = if let Some(addr) = accounts.get(instruction.accounts[0] as usize) {
        info!("✅ EXTRACTED POOL: {} | DEX: OrcaWhirlpools | From ix accounts[0]", addr);
        addr
    } else {
        warn!("❌ FAILED: ix.accounts[0]={} is out of bounds (accounts.len={})",
              instruction.accounts[0], accounts.len());
        return None;
    };
...
fn parse_meteora_dlmm_swap(
    ...
    if instruction.data.len() < 24 {
        warn!("⚠️  Meteora Parse REJECTED: instruction.data.len()={} < 24 required", instruction.data.len());
        return None;
    }
    ...
    if instruction.accounts.len() < 10 {
        warn!("⚠️  Meteora Parse REJECTED: instruction.accounts.len()={} < 10 required", instruction.accounts.len());
        return None;
    }
    ...
    let pool_idx = instruction.accounts.get(0)?;
    let pool_address = accounts.get(*pool_idx as usize);

    if pool_address.is_none() {
        warn!("⚠️  Meteora Parse REJECTED: accounts[{}] out of bounds (accounts.len={})",
              pool_idx, accounts.len());
        return None;
    }
    let pool_address = pool_address.unwrap();

    info!("✅ EXTRACTED POOL: {} | DEX: MeteoraDlmm | From ix accounts[2]={} | accounts.len={}",
          pool_address, pool_idx, accounts.len());
            ...
            if swap_size_sol >= config.min_swap_size_sol && swap_size_sol <= config.max_swap_size_sol {
                ...
                if is_profitable_to_sandwich(swap_size_sol, config) {
                    ...
                    let (pool_addr, input_mint, output_mint, amount_in, min_out) = if let Some(details) = swap_details {
                        ...
                        warn!("   Instruction accounts: {} accounts", instruction.accounts.len());
