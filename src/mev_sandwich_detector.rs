...
fn parse_raydium_clmm_swap(
    ...
    info!(
        "✅ EXTRACTED POOL: {} | DEX: Raydium_CLMM | From ix accounts[0]",
        pool_address
    );
...
fn parse_pumpswap_dex_swap(
    ...
    if instruction.data.len() < 25 {
        warn!(
            "⚠️  PumpSwap DEX Parse REJECTED: instruction.data.len()={} < 25 required",
            instruction.data.len()
        );
        return None;
    }
    ...
    if !is_buy && !is_sell {
        warn!(
            "⚠️  PumpSwap DEX Parse REJECTED: Unknown discriminator {:?}",
            discriminator
        );
        return None;
    }
    ...
    if instruction.accounts.len() < 11 {
        warn!(
            "⚠️  PumpSwap DEX Parse REJECTED: instruction.accounts.len()={} < 11 required",
            instruction.accounts.len()
        );
        return None;
    }
    ...
    info!(
        "✅ EXTRACTED POOL: {} | DEX: PumpSwap_DEX | From ix accounts[0]",
        pool_address
    );
